...
| 层 | 检查点 | 数据来源 | 是否始终执行 |
|---|---|---|---|
| 1.1 | 运营位是否生效 | 工具① 运营位查询 | 是 |
| 1.2 | 所属模块是否生效 | 工具① 模块查询(用 `belongToModuleId` 关联) | 是 |
| 2 | 是否通过平台限制(运营位 ∪ 模块,取并集) | 工具② 日志查询 · 第一步(`get-index-data` 日志，取「可见性过滤」结论) | 是 |
| 3 | 是否送入业务引擎 | 工具② 日志查询 · 第二步(引擎日志，用 `traceId` 关联，查 `feign-request`) | **仅当模块 `withEngine=true`** |
| 4 | 业务引擎是否返回 | 工具② 日志查询 · 第二步(同上，按 `modelCode` 分拣分流/客群脚本段) | **仅当模块 `withEngine=true`** |
| 5 | 客户端是否渲染 | 工具② 日志查询 · 客户端曝光日志(表名待补，见第五节) | 是 |
...
- 第①道配置校验 → **工具①** 运营位 + 模块查询接口
- 第②道入参校验 → **工具②** 日志查询 · 第一步(`get-index-data` 日志，取「可见性过滤」结论)
- 引擎段 → **工具②** 日志查询 · 第二步(用 `traceId` 关联，拉 `strategyA02` 分流日志 + 客群脚本日志两条，按 `modelCode` 分拣)

---

## 二、配置层判定(第 1 层)

> **数据来源：工具① 运营位 + 模块查询接口。** 先查运营位配置，再用 `belongToModuleId` 关联查所属模块配置，逐字段按下表判定。
